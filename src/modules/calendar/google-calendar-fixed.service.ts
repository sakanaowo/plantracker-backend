...
export class GoogleCalendarService {
  ...
  constructor(
    ...
    void this.initializeCalendarService();
  }

  private async initializeCalendarService() {
    try {
    ...
    try {
      ...
      if (existingMapping) {
        ...
        const result = await this.createEvent({
          title: event.title,
          description: `Event from PlanTracker project: ${event.projects?.name || 'Unknown'}`,
          startAt: event.start_at,
          endAt: event.end_at,
          location: event.location || undefined,
          attendees,
  ...
  async bulkSyncEvents(eventIds: string[]) {
    ...
    this.logger.log(
      `Bulk sync completed: ${results.filter((r) => r.success).length}/${results.length} successful`,
    );
    return results;
  }
